...
import { 
  ...
  Info, ExternalLink, GraduationCap, Download
} from "lucide-react";
...
export default function AlumniTab() {
  ...
  const handleExport = () => {
    ...
    window.location.href = `${baseUrl}/api/admin/export/alumni?token=${token}`;
  };
  ...
  const columns: Column<Alumni>[] = [
    ...
    { 
      key: "email", 
      label: "Email",
      render: (item: Alumni) => <span className="text-muted-foreground">{item.email || "—"}</span>
    },
    { 
      key: "batch", 
      label: "Batch",
      render: (item: Alumni) => <Badge variant="outline" className="font-mono text-[10px]">{item.batch || "N/A"}</Badge>
    },
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold tracking-tight">Alumni Network</h2>
                ...
                <div className="space-y-6">
                  ...
                  <section>
                    ...
                    <div className="flex items-center gap-2 text-xs text-muted-foreground italic">
                      ...
                      Member since {formatDate(selectedUser?.registered_at)}
                    </div>
                  </section>
                </div>
              </div>

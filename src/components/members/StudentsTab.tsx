...
import { 
  ...
  Mail, Phone, Calendar, Info, BookOpen, Download
} from "lucide-react";
...
export default function StudentsTab() {
  ...
  const columns: Column<Student>[] = [
    ...
    { 
      key: "email", 
      label: "Email",
      render: (item: Student) => <span className="text-muted-foreground">{item.email || "—"}</span>
    },
    { 
      key: "roll_number", 
      label: "Roll Number",
      render: (item: Student) => <span className="font-mono text-xs">{item.roll_number}</span>
    },
    {
      key: "semester",
      label: "Semester",
      render: (item: Student) => <Badge variant="secondary" className="font-mono">S-{item.semester}</Badge>,
    },
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold tracking-tight">Student Body</h2>
                  ...
                  <section>
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center justify-between text-xs p-2 bg-muted/50 rounded-lg">
                        <span className="flex items-center gap-2"><Calendar className="h-3.5 w-3.5" /> Registered</span>
